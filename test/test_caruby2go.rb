...
class TestCaruby2go < Minitest::Test
  ...
  def test_get_vehicles
    ...
    run_mock(vehicles_json, :get_vehicles)
  end

  def test_get_locations
    locations_json = '{"location":[{"countryCode":"DE","defaultLanguage":"de","locationId":1,"locationName":"Ulm","mapSection":{"center":{"latitude":48.398917,"longitude":9.99139},"lowerRight":{"latitude":48.3446,"longitude":10.0459},"upperLeft":{"latitude":48.4383,"longitude":9.9146} },"timezone":"Europe/Berlin"}]}'
    run_mock(locations_json, :get_locations)
  end

  def test_get_parkingspots
    parkingspots_json = '{"placemarks":[{"coordinates":[-97.750983,30.269577,0],"name":"West Ave","totalCapacity":4,"usedCapacity":0,"chargingPole":false},{"coordinates":[-97.74225,30.265976,0],"name":"100 East 4th Street","totalCapacity":4,"usedCapacity":0,"chargingPole":true}]}'
    run_mock(parkingspots_json, :get_parkingspots)
  end

  def test_get_gasstations
    gasstations_json = '{"placemarks":[{"coordinates":[9.987988,48.358829,0],"name":"Shell, Hauptstrasse 12"},{"coordinates":[9.990183,48.404832,0],"name":"Shell, Karlstrasse 38"}]}'
    run_mock(gasstations_json, :get_gasstations)
  end
